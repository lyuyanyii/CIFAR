...
def make_network(minibatch_size = 128, debug = False):
	...
	lmd = 1
	for i in lis_w:
		w = i.reshape(i.partial_shape[:3])
		w = w / ((w**2).sum(axis = 2)**0.5).dimshuffle(0, 1, 'x')
		A = O.BatchedMatMul(w, w.dimshuffle(0, 2, 1))
		I = O.ConstProvider(np.identity(A.partial_shape[2]))
		network.loss_var += lmd * ((A - I.dimshuffle('x', 0, 1))**2).mean()
