...
def res_layer(inp, chl, stride = 1, proj = False, se = None):
	...
	if se is None:
		...
		se = O.Concat([se, lay], axis = 1)
	lay = FullyConnected(
		"fc1({})".format(name), se, output_dim = chl * width,
		nonlinearity = Identity()
		)
	se = FullyConnected(
		"fc({})".format(se.name), se, output_dim = chl,
		nonlinearity = ReLU()
		)
...
def res_block(inp, chl, i, n, se):
	stride = 2
	if i == 0:
		stride = 1
	inp, se = res_layer(inp, chl, stride = stride, proj = True, se = se)
	
	for i in range(n - 1):
		inp, se = res_layer(inp, chl, se = se)
