...
def res_layer(inp, chl):
	...
	#fc0
	SE = FullyConnected(
		"fc0({})".format(name), SE, output_dim = (chl // group)**2 * group * ksize,
		nonlinearity = ReLU()
		)
	#fc1
	SE = FullyConnected(
		"fc1({})".format(name), SE, output_dim = (chl // group)**2 * group * ksize**2,
		nonlinearity = Sigmoid()
		)
	SE = SE - 0.5
	SE = SE.reshape(inp.shape[0] * group, chl // group, chl // group, ksize, ksize)
	#inp = inp * SE.dimshuffle(0, 1, 'x', 'x')
	inp = inp.reshape(1, inp.shape[0] * inp.shape[1], inp.shape[2], inp.shape[3])
	inp = Conv2D(
		"conv({})".format(name), inp, kernel_shape = ksize, stride = 1, padding = 1,
