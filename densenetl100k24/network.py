...
def dense_layer(inp, k, B):
	if not B:
		return bn_relu_conv(inp, 3, 1, 1, k, True, True)
	else:
		inp = bn_relu_conv(inp, 1, 1, 0, k * 4, True, True)
		inp = bn_relu_conv(inp, 3, 1, 1, k, True, True)
		return inp

def dense_block(inp, k, l, B):
	lay = inp
	for i in range(l):
		cur_lay = dense_layer(lay, k, B)
...
def transition(inp, i):
	...
	if i != 2:
		...
		window, stride = 8, 8
	l2 = Pooling2D(
		"Pooling{}".format(idx), l1, window = window, stride = stride, mode = "AVERAGE"
		)
	return l2
