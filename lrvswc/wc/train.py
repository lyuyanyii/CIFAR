...
if __name__ == '__main__':
	...
	with TrainingEnv(name = "lyy.{}.test".format(net_name), part_count = 2, custom_parser = parser) as env:
		...
		net.loss_var = MyWeightDecay(net.loss_var, {"*conv*:W": 1e-4, "*fc*:W": 1e-4, "*bnaff*:k": 1e-4, "*offset*":1e-4})
		"""
			...
			with control(io = [va_p]):
				...
				while i <= TOT_IT:
					...
					if i == ORI_IT // 2 or i == ORI_IT // 4 * 3:
						"""
						a = net.loss_var.owner_opr._param_weights
						a = [i / 10 for i in a]
						net.loss_var.owner_opr._param_weights = a
						"""
						#net.loss_var.owner_opr.Mul_Wc(0.1)
						optimizer.learning_rate /= 10
					if i == ORI_IT:
						#optimizer.learning_rate = 1e-5
						#net.loss_var.owner_opr.Mul_Wc(0.01)
