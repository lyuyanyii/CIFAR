...
def dense_block(inp, k, l):
	...
	for i in range(l):
		...
		sh_lay = lay.reshape(lay.partial_shape[0], chl // k, k, lay.partial_shape[2], lay.partial_shape[3])
		sh_lay = sh_lay.dimshuffle(0, 2, 1, 3, 4)
		sh_lay = sh_lay.reshape(lay.shape)
		cur_lay = bn_relu_conv(sh_lay, 3, 1, 1, k, True, True, group = k // 4) + pre_lay
		pre_lay = bn_relu_conv(cur_lay, 1, 1, 0, k, True, True)
