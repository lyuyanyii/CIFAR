...
def make_network(minibatch_size = 128):
	...
	for lay, idx in zip(mid_lays, range(len(mid_lays))):
		lay, conv = conv_bn(lay, 3, 1, 1, lay.partial_shape[1], True)
		feature = lay.mean(axis = 2).mean(axis = 2)
		pred = Softmax("pred_{}".format(idx), FullyConnected(
			"fc_{}".format(idx), feature, output_dim = 10,
			nonlinearity = Identity()
			))
		network.loss_var += 0.5 * CrossEntropyLoss(pred, label, name = "loss{}".format(idx))
