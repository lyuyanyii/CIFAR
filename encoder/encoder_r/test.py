...
def img_test():
	...
	tcnn = NN("./data/encoder_r.data")
	for i in range(10):
		img, lab = data[i], labels[i]
		img = np.array(img, dtype = np.uint8).reshape(3, 32, 32)
		img = img.transpose(1, 2, 0)
		outputs = tcnn.test([img], [lab])
		img_out = outputs[0][0]
		img_out = img_out * tcnn.std + tcnn.mean
		img_out = img_out.astype(np.uint8)
		img_out = img_out.transpose(1, 2, 0)
		cv2.imwrite("imgs/{}_i.png".format(i), img)
		cv2.imwrite("imgs/{}_o.png".format(i), img_out)
